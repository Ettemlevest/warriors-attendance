  ...
  <layout title="Dashboard">
    ...
    <div v-for="training in trainings" :key="training.id" class="overflow-x-auto block lg:flex-wrap xl:flex-wrap">
      <div class="text-center sm:w-auto md:w-auto lg:flex-1 xl:flex-1 m-4 p-4 bg-white rounded-lg shadow-md">
        ...
        <button v-if="!training.registered && training.attendees < training.max_attendees" class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 mt-4 rounded">
          Jelentkezem
        </button>
        <div v-if="!training.registered && training.attendees >= training.max_attendees" class="bg-red text-white font-bold py-2 px-4 mt-4 rounded">Megtelt, már nem lehet jelentkezni!</div>
        <button v-if="training.registered" class="bg-red hover:bg-red-dark text-white font-bold py-2 px-4 mt-4 rounded">
          Lemondás
        </button>
      </div>
    </div>
