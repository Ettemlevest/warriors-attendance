  ...
  <layout title="Dashboard">
    ...
    <div v-for="training in trainings" :key="training.id" class="overflow-x-auto block lg:flex-wrap xl:flex-wrap">
      <div class="text-center sm:w-auto md:w-auto lg:flex-1 xl:flex-1 m-4 p-4 bg-white rounded-lg shadow-md">
        <h2 class="text-xl mb-2 ml-8 mr-8">{{ training.name }}</h2>
        ...
        <div class="sm:block md:block lg:flex xl:flex lg:items-center">
          ...
          <div class="text-grey-darker items-center flex lg:flex-1 xl:flex-1 text-center">
            <icon name="dashboard" class="flex w-5 h-5 fill-grey mr-2" />
            {{ training.start_at }}
          </div>
        </div>
        <button class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 mt-4 rounded">
          Jelentkezem
        </button>
        <div class="bg-red text-white font-bold py-2 px-4 mt-4 rounded">Megtelt, már nem lehet jelentkezni!</div>
        <button class="bg-red hover:bg-red-dark text-white font-bold py-2 px-4 mt-4 rounded">
          Lemondás
        </button>
      </div>
    </div>
